...
Mit dem Code aus Listing \ref{lst:azurepublish} kann der Code dann an andere \ac{ROS} Nodes nach dem \ac{ROS} Publish und Subscribe Pattern veröffentlicht werden.
...
\begin{figure}
\begin{python}[breaklines=true, label=lst:orbslammavros]
import rospy
...
rospy.spin()
\end{python}
\lstlistingname{ ORB-SLAM3 Position zu MAVROS senden}
\end{figure}



\begin{figure}
\begin{python}[breaklines=true,label=lst:azurepublish]
...
\end{python}
\lstlistingname{ Azure Kinect mit ORB-SLAM3 verbinden}
\end{figure}
...
\begin{roslaunch}[breaklines=true, label=lst:mavroslaunch]
    <launch>
...
Die Simulation ist in großen Zügen bereits von Clover vordefiniert. Im ROS Package clover Simulation was im Github Repo \cite{coex_clover_git} zu finden ist, befinden sich die Launch Files. Es sind sowohl Launch Files für die Simulation, wie auch für die direkte Verwendung auf der Drohne vorhanden.
